...
//            views.ErrorPage.goToErrorPage("Wrong X");
        ...
        //Проверка Y
        try {
            if (dotForExecution.getY() != null && dotForExecution.getY() >= -5 && dotForExecution.getY() <= 3) {
...
//            views.ErrorPage.goToErrorPage("Wrong Y");
        ...
        currentDot.setAtArea(AreaResult.isItInArea(currentDot) ? "Hit" : "Miss");
    ...
    public void addDotFromSvg() throws IOException {
        ...
        currentDot.setAtArea(AreaResult.isItInArea(dotFromSVG) ? "Hit" : "Miss");
        if (dbManager.addPoint(currentDot)) {
            dotsList.add(currentDot);
        }
    }
